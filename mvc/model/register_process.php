<?php
    ...
    if (isset($_POST['btn'])) {
        ...
        $phone = $_POST['phone']??"";
            ...
            // check valuedation form
            if (empty($name) or empty($pwd) or empty($fullname) or empty($email) or empty($pwd_comfirm)) {
                $error = 'Bạn phải điền hết thông tin (Tên đăng nhập, mật khẩu, Tên đầy đủ và email!)';
            } else {
                if (strlen($usr) < 4 or strlen($usr) > 50) {
                    $error = 'Tài khoản phải từ 4-50 ký tự!';
                }
                if (strlen($pwd) < 4 or strlen($pwd) > 50) {
                    $error = 'Mật khẩu phải từ 4-50 ký tự!';
                }
                if (strlen($fullname) < 4 or strlen($fullname) > 50) {
                    $error = 'Tên của bạn phải từ 4-50 ký tự!';
                }
                ...
                if ($pwd != $pwd_comfirm) {
                    $error = 'PassWord Khong Trung Khop';
                }
            }
